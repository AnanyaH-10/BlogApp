...
const BlogDetails = () => {
    ...
    const getBlogDetail = async()=>{
        try {
            const {data} = await axios.get(`https://blogapp-backend-r9uc.onrender.com/api/v1/blog/get-blog/${id}`)
            if(data?.success){
                setBlog(data?.blog)
            }
            setInputs({
    ...
    const handleSubmit=async (e)=>{
        e.preventDefault();
        try {
            const {data} = await axios.put(`https://blogapp-backend-r9uc.onrender.com/api/v1/blog/update-blog/${id}`,{
                title:inputs.title,
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
        <Box width={'80%'} border={3} borderRadius={10} padding={3} margin="auto" boxShadow={'10px 10px 20px #ccc'} display="flex" flexDirection={'column'} marginTop={'30px'}>
